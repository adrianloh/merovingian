...
# Also, we declare it here to enforce facility-wide conventions
# ---------------------------------------------------------------------- #

import os
import time
import re
import __builtin__ # We want to write into Python's topmost-level "built in" namespace
from subprocess import Popen,PIPE

__builtin__.compStatusItems = ["Setup","WIP","Review","Qrender","Rendering","Attention","Locked","Publish","Milestone","Incomplete","Abandoned"]
__builtin__.renderStatusItems = ["Qrender","Rendering","Review","Lock","Save"]

os.environ["JOBS"] = "G:/jobs"

__builtin__.path_resolution_order = ["JOBS","SHOTROOT","HOME"]
...
while not allMetaPresent:
	try:
		...
		nuke.root()['JOB'].value()
		nuke.root()['SHOT'].value()
		nuke.root()['SHOTROOT'].value()
		nuke.root()['browseroot'].value()
		...
		nuke.root().addKnob(nuke.PyScript_Knob("browseroot","Set shot folder","getShotFolder()"))
		nuke.root().addKnob(nuke.Int_Knob("versionMajor","versionMajor"))
		nuke.root().addKnob(nuke.Int_Knob("versionMinor","versionMinor"))
		nuke.root().addKnob(nuke.Enumeration_Knob("status","status",compStatusItems))

def getShotFolder():
	def split(p): return re.split(r"[\\\\|/]",p)
	def join(p): return "/".join(p)[:-1]
	p = nuke.getFilename("Select shot folder...", pattern="/")
	if not p: return
	p = split(p)
	for e in path_resolution_order:
		base = split(os.environ[e])
		pp = [c for c in p if c not in base]
		nuke.root()['SHOTROOT'].setValue(join(pp))
	
__builtin__.getShotFolder = getShotFolder

def checkRootMetadata():
	
	if nuke.root()['title'].value() == "":
		nuke.root()['title'].setValue("MAIN")
	
	if nuke.root()['JOB'].value() == "":
		job = nuke.getInput("Enter Project name:")
		job = job if job else "LAB"
		nuke.root()['JOB'].setValue(job)

	if nuke.root()['SHOT'].value() == "":
		shot = nuke.getInput("Enter Shot name:")
		shot = shot if shot else "SANDBOX"
		nuke.root()['SHOT'].setValue(shot)

def loadRootMetadata():
	if nuke.root()['SHOTROOT'].value():
		os.environ["SHOTROOT"] = nuke.root()['SHOTROOT'].value()
	else:
		os.environ['SHOTROOT'] = os.environ['JOBS']
...
def customExecuteMultiple(_nodes,ranges=None,views=['main'],default_order=False):
	...
	# Note that we have commented out a line from nukescripts/execute.py to
	# no longer prompt us for framerange because that is determined
	# on a node-to-node basis in customized Write "Render Range" field

	if type(_nodes) is list:
		# "Render Selected" passes in the usual expected list of nodes
		nodes = _nodes
	elif nuke.selectedNodes("Write").__len__()==1:
		# When the "Render" button is clicked in an opened Write panel
		nodes = nuke.selectedNodes("Write")
	else:
		# Lazy checking, if "Render All" is invoked when nothing is selected then -- assume we want all the Write nodes
	...
	if disabled:
		msg = "RENDER ERROR: Write nodes disabled: %s \n" % " ".join([d.name() for d in disabled])
	...
	if reading:
		msg = "RENDER ERROR: Write nodes in Read mode: %s \n" % " ".join([r.name() for r in reading])
		sys.stderr.write(msg)
...
def tweet(message,force=False):
	...
	cmd = 'curl --silent --user Hiroshima_V1:yesterday --data status="%s" %s' % (message,url)
	if time.clock()-last_tweet_time > 90 or force:
		po = Popen(cmd,stdout=PIPE).stdout	# It's this elaborate to suppress messages
...
def renderStatus2Twitter(interval=10):
	...
	every n-th interval frame. 	Usually attached to nuke.addAfterFrameRender.
	"""
...
REMEMBER: To use this, you must specifically use this format:
	...
	nuke -it /path/to/script.nk -renderonly "Write1,Write4,Write5" !! Comma delimited

THE EVAL/EXEC SWITCH

The -eval and -exec switches allow you to interrogate/change nuke scripts as
if you were on a command line with nuke -t in the form:

	nuke -it /path/to/script.nk -eval "statement"
 
Use -eval to get data, use -exec to set data. 

E.g. to change the framerate of a comp and save you would use -exec:

	nuke -it /path/to/script.nk -exec "nuke.root()['fps'].setValue(30) ; nuke.scriptSave("nuke.root().name())" 
...
All returned values of eval are passed to print() to STDOUT so you'll have to 
be smart about formatting data into a string.
 
Implementation include creating an interrogation function in a separate 
Python script. This is how you do it, with all the code needed to clean 
the junk printed to STDOUT

	1. Issue command via Popen
	2. Get the last line from STDOUT (what we actually want), the rest is Foundry's Copyright junk
	3. strip away /r/n characters
	4. eval() the text, which turns it back either to Python text (redundant) or a usable array!

	from subprocess import Popen,PIPE
	cmd = "" nuke -it /path/to/script.nk -eval "[nuke.filename(n) for n in nuke.allNodes('Write')]" ""
	po = Popen(cmd,stdout=PIPE).stdout.readlines()[-1].strip()
	x = eval(po)
	x[0] # Works!

In any case, note that it's probably easier to just import the nuke module
from within your script

"""

if not nuke.env['gui']:
	f = [n for n in sys.argv if re.search("\.nk",n)] # get path to file from arguments
	Q = None
	if "-renderall" in sys.argv:
		...
		if None in Q:
			...
			msg = "RENDER ERROR -renderonly failed to find nodes: %s \n" % " ".join(fail)
			sys.stderr.write(msg)
			Q = [nuke.toNode(n) for n in write_names if n in ok]
		nuke.executeMultiple(Q)
		code = 1 if missingNodes else 0
		sys.exit(code)
