...
def customExecuteMultiple(nodes,ranges=None,views=['main'],default_order=False):
	"""
	...
	if default_order:
		...
		for n in nodes:
			try:
				...
				attach_range(n)
		if missing_range:
			cont = nuke.ask("Range knobs were added to Write nodes\n\
...
def nk_backburner():
	...
	cmd = "nuke -m %i -F %s -X %s -x %s" % (int(nuke.env['numCPUs']/2),ranges,name,np)
	
	def render():
		from nukescripts import utils
		Popen(cmd).wait()
		nuke.toNode(name)['tile_color'].setValue(536805631) # Why ain't this working?
		utils.executeInMainThread(nuke.toNode(name)['reading'].setValue,True)
		os.remove(np)

	nuke.toNode(name)['reading'].setValue(False)
	nuke.toNode(name)['tile_color'].setValue(1996488704)
...
def file(oper):
	...
	if first and last:
		...
		for (i,f) in enumerate(files):
			...
			print "%s --> %s" % (op,np)
			shutil.copy2(op,np)
			if oper == 1: # Move operation
				os.remove(op)
			task.setMessage("%s:%s"%(subMsg,op))
			task.setProgress(i/T)
