...
RVPATH = "C:\\bin\\rv.cmd"
NUKE_PATH ="C:\\Program Files\\Nuke6.0v1\\Nuke6.0.exe"
...
def gc_saveToDB():
	...
	if Projects.get(id=pid)[0] is None:
		nuke.message("Mother doesn't know about this project!")
		gc_newProject()
		return
	elif Shots.get(project_id=pid,id=sid)[0] is None:
		nuke.message("Let mother know about this shot first!")
		gc_newShot(sid,r['SHOTROOT'].value())
		return
	else:
		nuke.scriptSaveAs(randstring(15),overwrite=1)
		p = nuke.root().name()
		s = open(p).read()
		c = Nuke_versions(	title=r['title'].value(),
							project_id=r['JOB'].value(), shot_id=r['ID'].value(), shotroot=r['SHOTROOT'].value(),
							versionMajor=r['versionMajor'].value(), versionMinor=r['versionMinor'].value(),
							modified=datetime.datetime.now(),status=r['status'].value(),
							comments=r['label'].value(),
							script=s
							)
		c.save()
...
def attach_write_status(n):
	n.addKnob(nuke.Enumeration_Knob("status","status",["Qrender","Rendering","Review","Lock"]))
	return n
...
def customizeNodeOnUserCreate():
	...
	if nClass == "Read":
		attach_file_operations(n)
		n['update_source'].setValue(1)

	if nClass == "Write":
		n['channels'].setValue("rgba")
		attach_range(n)
		attach_switch_routing(n)
		attach_write_status(n)
		attach_file_operations(n)
		n['update_source'].setValue(1)
