...
GCMenu = toolbar.addMenu('Grand Central')
...
for giz in [os.path.splitext(n)[0] for n in os.listdir(os.environ["DOTNUKE"]) if re.search("gizmo",n)]:
...
for func in [f for f in dir(nk) if f.startswith("gc")]:
	f = eval("nk.%s" % func)
	label = [l.strip() for l in f.__doc__.split("\n")][1]
