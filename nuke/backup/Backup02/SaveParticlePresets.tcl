...
proc SaveParticlePresets {presetName} {	
	...
	set cur_file "[getenv HOME]/.nuke/$new_name"
	if [file exists $cur_file] {
		if ![ask "Overwrite the following file?\n$cur_file"] {return}
		}
	set chan [open $cur_file w]
	set skip_knobs {name label tile_color gl_color note_font note_font_size note_font_color lock_connections knob knob_1 preset_file presets mapsize normCurve}
