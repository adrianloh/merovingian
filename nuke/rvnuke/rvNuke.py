...
class RvMonitor :
    ...
    def knobChanged (self) :
        ...
        #
        elif (self.syncReadChanges and (str(node.Class()) == "Read" or str(node.Class()) == "Write") and self.running and (
                knob.name() == "colorspace" or
                knob.name() == "first" or
                knob.name() == "last" or
                knob.name() == "label" or
                knob.name() == "frame_mode" or
                knob.name() == "frame" or
                knob.name() == "file")) :
    ...
    def initRenderRun (self, outputNode, startFrame, endFrame, incFrame, audioFile, audioOffset, cpFrame, label, date, renderType, stereo) :
        """
        ...
        """ % (outputNode, startFrame, endFrame, incFrame, audioFile, audioOffset, cpFrame, self.outputFileFormat, label, date, renderType, stereo))
    ...
    def removeObsoleteReads (self) :
        ...
        readNodes = [ n.name() for n in nuke.allNodes() if (type(n).__name__ == "Node" and 
                (n.Class() == "Read" or n.Class() == "Write")) ]
        if (not readNodes) :
            readsStr = "string[] {}" 
        else :
    ...
    def viewReadsInRv (self, readNodes) :
        ...
        remote = "require rvnuke_mode; rvnuke_mode.theMode().viewReadNodes (%s, %s, %s, %s, %s, %s, %s, %s);" % (
                nameStr, mediaStr, spaceStr, firstStr, lastStr, labelStr, classStr, offsetStr)
...
class RvSettings() :
    ...
    def rebuildSettingsKnobs (self, r) :
        ...
        knobList += [ "rvSettings_syncOptionsStart", "rvSettings_syncOptionsEnd", "rvSettings_RVstart" ]
...
class RvPreferencesPanel (nukescripts.PythonPanel) :
    ...
    def __init__( self):
        ...
        self.sessionDirBase.setTooltip ("Path to base directory from which to form sessionDirectories. $NUKE_TEMP_DIR will be used if this is not set.")
    ...
    def knobChanged (self, knob) :
        ...
        if (self.rvPrefs.prefs.has_key (knob.name())) :
...
def viewReadsInRv (all) :
    ...
    readNodes = [ n.name() for n in nodes if (type(n).__name__ == "Node" and 
            (n.Class() == "Read" or n.Class() == "Write")) ]
